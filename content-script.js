    ...
    async function startExtraction() {
      ...
      try {
        ...
        const productData = {
          ...
          descriptionImage: `${data.productName}/${data.material || '기타'}/${data.model}/${descFileName}`,
          images: data.imageUrls.map((url, idx) => {
            const urlParts = url.split('.');
            const extension = urlParts.length > 1 ? urlParts[urlParts.length-1].split('?')[0] : 'png';
    ...
    function saveProductData(productData) {
      chrome.storage.local.get(['products'], (result) => {
        ...
        chrome.storage.local.set({ products }, () => {
          ...
          const imageMsg = productData.images.length > 0 
            ? `이미지 ${productData.images.length}개 다운로드 중` 
            : '이미지 없음';
