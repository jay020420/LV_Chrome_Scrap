// URL 확인기 스크립트 동적 import
async function importUrlStockChecker() {
  try {
    // 스크립트 가져오기 (ES modules가 Manifest V3에서 지원됨)
    const script = await import('./url-based-stock-checker.js');
    
    // 글로벌 스코프에 추가
    self.URLStockChecker = script.URLStockChecker || script.default;
    
    console.log('URL 확인기 스크립트 로드 완료');
  } catch (error) {
    console.error('URL 확인기 스크립트 로드 실패:', error);
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // 자동 추적 설정 변경
  else if (message.action === 'setAutoTracking') {
    trackingStatus.autoTracking = message.enabled;
    
    // 설정 저장
    chrome.storage.local.set({ autoTracking: message.enabled }, function() {
      console.log('자동 추적 설정 변경:', message.enabled);
  ...
  // 추적 주기 설정 변경
  else if (message.action === 'setTrackingInterval') {
    trackingStatus.trackingInterval = message.interval;
    
    // 설정 저장
    chrome.storage.local.set({ trackingInterval: message.interval }, function() {
      console.log('추적 주기 설정 변경:', message.interval);
      
      // 자동 추적이 활성화되어 있으면 일정 재설정
...
async function initUrlChecker(settings) {
  try {
    ...
    // URLStockChecker가 정의되지 않았거나 오류 발생 시
    console.log('URLStockChecker 클래스를 직접 정의합니다.');
    
    // 기본 URLStockChecker 클래스 만들기
    class URLStockChecker {
      constructor() {
        ...
        this.settings = { delayMin: 3, delayMax: 7, batchSize: 1, maxRetries: 2 };
        this.callbacks = {};
      }
      
      async init(type) {
        // URL 목록 로드 로직...
        console.log('URL 목록 초기화...');
    ...
    throw error; // 원래 오류를 다시 던져서 상위 레벨에서 처리되도록 함
  }
}
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // URL 확인기 초기화: URLStockChecker 스크립트 로드
  importUrlStockChecker();
});
